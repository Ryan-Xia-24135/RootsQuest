          ...
          <AccordionItem value={item.id} key={item.id} className="border-white/25 last:border-b">
            <AccordionTrigger className="group cursor-pointer overflow-hidden px-[clamp(12px,1.8vw,28px)] py-[clamp(9px,min(1.45vw,2.15vh),18px)] text-left text-white/60 duration-200 hover:text-white hover:no-underline data-[state=open]:rounded-t-[14px] data-[state=open]:bg-[#ccdecd] data-[state=open]:text-[#112e1b] max-sm:px-3 max-sm:py-2 [&>svg:last-child]:hidden">
              <div className="flex min-w-0 flex-1 items-center gap-[clamp(10px,1.55vw,24px)]">
                ...
                <h2 className="m-0 min-w-0 flex-1 font-['Asta_Sans',Arial,sans-serif] text-[clamp(18px,min(3vw,4.8vh),46px)] font-semibold leading-[1.08] tracking-[-0.03em] max-sm:text-[clamp(16px,5vw,21px)] portrait:text-[clamp(18px,5.5vw,24px)]">{item.title}</h2>
            ...
            <AccordionContent className="rounded-b-[14px] bg-[#ccdecd] px-[clamp(40px,6vw,92px)] pb-[clamp(12px,min(2vw,2.8vh),24px)] text-[#23452f] max-sm:px-4 max-sm:pb-3">
              <p className="m-0 max-h-[18vh] overflow-y-auto font-['Inter',Arial,sans-serif] text-[clamp(11px,min(1.7vw,2.9vh),26px)] font-normal leading-[1.35] tracking-[-0.02em] portrait:text-[15px]">{item.content}</p>
            </AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
