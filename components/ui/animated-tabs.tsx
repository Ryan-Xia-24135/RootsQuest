    ...
    <div className="relative mx-auto flex w-fit items-center rounded-full border border-white/15 bg-black/35 px-[clamp(3px,0.35vw,5px)] py-[clamp(2px,0.25vw,4px)] backdrop-blur-[2px]">
      ...
      >
        <div className="flex h-full w-full items-center bg-white">
          {tabs.map((tab) => (
            <span
              className="flex h-[clamp(22px,min(2.25vw,3.4vh),30px)] items-center whitespace-nowrap rounded-full px-[clamp(6px,min(0.85vw,1.35vh),12px)] font-['Asta_Sans',Arial,sans-serif] text-[clamp(8px,min(1.05vw,1.8vh),14px)] font-medium leading-none text-[#112e1b]"
              key={tab.href}
            >
              {tab.label}
            </span>
        ...
        {tabs.map((tab) => {
          ...
          return (
            <Link
              ...
              className="flex h-[clamp(22px,min(2.25vw,3.4vh),30px)] items-center whitespace-nowrap rounded-full px-[clamp(6px,min(0.85vw,1.35vh),12px)] font-['Asta_Sans',Arial,sans-serif] text-[clamp(8px,min(1.05vw,1.8vh),14px)] font-medium leading-none text-white/80 transition-colors hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
            >
              {tab.label}
            </Link>
          );
