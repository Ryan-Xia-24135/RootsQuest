...
function EmptyPhotoBlock({ label }: { label: string }) {
  return (
    <div
      className="min-h-0 rounded-[clamp(18px,2vw,30px)] border border-dashed border-white/35 bg-black/15 shadow-[0_24px_60px_rgba(0,0,0,0.25)] backdrop-blur-[2px] max-sm:hidden"
      role="img"
      aria-label={label}
    />
  );
...
export function CourseConnectionContent() {
  return (
    <div className="absolute inset-x-0 bottom-0 top-[var(--nav-height)] z-10 overflow-hidden p-[clamp(14px,3vw,46px)] text-white max-sm:p-3">
      <div className="mx-auto grid h-full max-w-[1480px] grid-cols-[0.86fr_1.14fr] gap-[clamp(14px,2.4vw,36px)] max-sm:flex max-sm:flex-col max-sm:justify-center">
        <section className="flex min-h-0 flex-col justify-between rounded-[clamp(18px,2vw,30px)] border border-white/15 bg-black/35 p-[clamp(20px,3.2vw,48px)] shadow-[0_28px_70px_rgba(0,0,0,0.32)] backdrop-blur-[3px] max-sm:p-4">
          <div>
            <span className="mb-[clamp(10px,1.7vh,17px)] block h-px w-[clamp(62px,8vw,112px)] bg-[#c8a84b]" />
            <h1 className="m-0 font-['Asta_Sans',Arial,sans-serif] text-[clamp(28px,min(4.4vw,6.3vh),66px)] font-semibold leading-[1.06] tracking-[-0.03em] max-sm:text-[clamp(26px,8vw,40px)] portrait:text-[clamp(30px,9vw,46px)] portrait:font-bold">
              The Connection Between Everything
            </h1>
            <p className="mt-[clamp(14px,2.3vh,24px)] max-w-[38ch] font-['Inter',Arial,sans-serif] text-[clamp(13px,min(1.75vw,3vh),26px)] font-normal leading-[1.45] tracking-[-0.02em] max-sm:mt-3 max-sm:max-w-none max-sm:text-[15px] portrait:text-[17px] portrait:leading-[1.5]">
          ...
          <p className="m-0 rounded-[clamp(14px,1.5vw,22px)] border border-[#dac6b6]/35 bg-[#9c7561]/85 p-[clamp(16px,2.5vw,36px)] font-['Inter',Arial,sans-serif] text-[clamp(12px,min(1.55vw,2.6vh),23px)] font-normal leading-[1.42] text-white shadow-[0_18px_45px_rgba(0,0,0,0.25)] max-sm:mt-4 max-sm:p-4 max-sm:text-[15px] portrait:text-[16px]">
    ...
    <div className="absolute inset-x-0 bottom-0 top-[var(--nav-height)] z-10 overflow-hidden p-[clamp(14px,3vw,46px)] text-white max-sm:p-3">
      <div className="mx-auto flex h-full max-w-[1480px] min-h-0 flex-col rounded-[clamp(18px,2vw,30px)] border border-white/15 bg-black/30 p-[clamp(18px,3vw,44px)] shadow-[0_28px_70px_rgba(0,0,0,0.32)] backdrop-blur-[3px] max-sm:justify-center max-sm:p-3">
        <header className="shrink-0 text-center">
          ...
          <h1 className="m-0 font-['Asta_Sans',Arial,sans-serif] text-[clamp(27px,min(4.25vw,6vh),64px)] font-semibold leading-[1.08] tracking-[-0.03em] max-sm:text-[clamp(25px,7.7vw,38px)] portrait:text-[clamp(28px,8.5vw,42px)] portrait:font-bold">
          ...
          {outcomes.map((outcome, index) => (
            ...
            >
              <div className="p-[clamp(14px,1.8vw,27px)]">
                <h2 className="m-0 font-['Asta_Sans',Arial,sans-serif] text-[clamp(18px,min(2.25vw,3.8vh),34px)] font-semibold leading-[1.1] tracking-[-0.03em] portrait:text-[22px] portrait:font-bold">{outcome.title}</h2>
                <p className="mt-[clamp(8px,1.3vh,13px)] font-['Inter',Arial,sans-serif] text-[clamp(10px,min(1.14vw,2vh),17px)] font-normal leading-[1.4] max-sm:text-[15px] portrait:text-[16px]">{outcome.description}</p>
              </div>
...
export function CourseInstructorContent({ theme = "courses" }: { theme?: "courses" | "about" }) {
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 top-[var(--nav-height)] z-10 overflow-hidden p-[clamp(14px,3vw,46px)] text-white max-sm:overflow-y-auto max-sm:p-3">
      <div className="mx-auto grid h-full max-w-[1480px] grid-cols-[0.92fr_1.16fr_0.92fr] gap-[clamp(12px,2vw,30px)] max-sm:flex max-sm:h-auto max-sm:min-h-full max-sm:flex-col max-sm:justify-center max-sm:gap-3">
        <section className={`flex min-h-0 flex-col rounded-[clamp(18px,2vw,30px)] border p-[clamp(18px,2.8vw,42px)] shadow-[0_28px_70px_rgba(0,0,0,0.32)] backdrop-blur-[3px] max-sm:p-4 ${isAbout ? "border-[#b9d4c0]/30 bg-[#102d21]/78" : "border-white/15 bg-black/35"}`}>
          <span className="mb-[clamp(10px,1.6vh,16px)] block h-px w-[clamp(58px,7vw,104px)] bg-[#c8a84b]" />
          <h1 className="m-0 font-['Asta_Sans',Arial,sans-serif] text-[clamp(26px,min(3.5vw,5.5vh),52px)] font-semibold leading-[1.08] tracking-[-0.03em] max-sm:text-[clamp(22px,7vw,30px)] portrait:text-[clamp(26px,7.5vw,36px)] portrait:font-bold">About the Instructor</h1>
          <p className="mt-[clamp(14px,2.2vh,22px)] min-h-0 overflow-y-auto font-['Inter',Arial,sans-serif] text-[clamp(11px,min(1.36vw,2.35vh),20px)] font-normal leading-[1.45] max-sm:mt-3 max-sm:text-[15px] portrait:text-[16px]">
